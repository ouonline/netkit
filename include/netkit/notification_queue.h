...
public:
    ...
    /** @brief receives at most `sz` bytes into `buf` from `fd`. returns 0 or -errno. */
    virtual int RecvAsync(int64_t fd, void* buf, uint64_t sz, void* tag) = 0;

    /**
       @brief sends at most `sz` bytes from `buf` to `fd`, with an additional flag
       `MSG_NOSIGNAL`. returns 0 or -errno.
    */
    ...
    /**
       ...
       - RECV/READ: `res` is the number of bytes received or -errno, `tag` is the value
                    passed to `RecvAsync()`.
       - SEND/WRITE: `res` is the number of bytes sent or -errno, `tag` is the value
                     passed to `SendAsync()`.
