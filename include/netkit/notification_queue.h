...
public:
    ...
    /** @brief sends at most `sz` bytes from `buf` to `fd`. returns 0 or -errno. */
    ...
    /**
       ...
       - RECV: `res` is the number of bytes received or -errno, `tag` is the value
               passed to `RecvAsync()`.
       - SEND: `res` is the number of bytes sent or -errno, `tag` is the value
               passed to `SendAsync()`.
